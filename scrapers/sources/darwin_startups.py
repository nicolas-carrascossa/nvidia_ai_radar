...
_MODEL = "gpt-4o-mini"
_PORTFOLIO_URL = "https://www.darwinstartups.com/startups/"
_FONTE_ORIGEM = "darwin_startups"
...
class DarwinStartupsScraper(BaseScraper):
    ...
    def discover(self) -> list[dict]:
        page = self._firecrawl.scrape(_PORTFOLIO_URL)
        if not page:
            print(f"[DarwinStartupsScraper] sem conteudo em {_PORTFOLIO_URL}")
            return []
        ...
        try:
            ...
            print(f"[DarwinStartupsScraper] erro LLM: {e}")
            return []
        ...
        for item in items:
            try:
                ...
                print(f"[DarwinStartupsScraper] erro ao processar item {item}: {e}")
